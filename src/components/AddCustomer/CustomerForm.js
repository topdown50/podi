...
import CustomerFormMain from './CustomerFormMain'

export default function CustomerForm({title,
  customer,
  setCustomer,
  billAdd,
  setBillAdd,
  ...
  return (
    <div className='p-0 p-md-4'>
      ...
      <CustomerFormMain customer={customer} setCustomer={setCustomer} setDisable={setDisable}/>
      ...
      {billAdd.map((billAddItem, index) =>(
        ...
        <div className='mx-auto d-flex justify-content-center mb-4'>
            <button onClick={saveCustomer} type="button" className="btn btn-success btn-lg" hidden={setDisable}>Submit</button>
